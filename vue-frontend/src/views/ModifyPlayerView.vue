		...
		if (cache !==  null) {
		  cache = JSON.parse(cache);
			...
			if (TwentyThreeHoursAgo(connected_at)) {
			  ...
			  console.log(this.postData, this.$route.params.id, this.$route.params.name);
			}
		}
	},
  data() {
	  ...
	  async Logout() {
		...
		async PostPlayer()
		{
		  ...
		  const post_data = {
			...
			let response = '';
			try {
				response = await axios.put(PLAYERS_URL + this.$route.params.id, post_data,  {
					headers: {
